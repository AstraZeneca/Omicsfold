...
blockrank.diablo <- function(diablo.model) {
    ...
    blockrank.i <- .internal.blockrank.calculation(x.data, y.data, x.loadings, y.loadings, ncomp, nblock)
...
#'
.internal.blockrank.calculation <- function(x.data, y.data, x.loadings, y.loadings, ncomp, nblock){
    X.h <- x.data
    Y_model <- matrix(0, nrow = nrow(y.data), ncol = ncol(y.data))
    Xt_model <- lapply(x.data,
                       function(x) matrix(0, nrow = ncol(x), ncol = nrow(x)))
    ...
    for (h in seq_len(ncomp)){
        ...
        for (q in seq_len(nblock)) {
            ...
            Xt_model[[q]] <- Xt_model[[q]] + x.loadings[[q]][, h] %*% t(X.a)
    ...
    blockrank.i <- lapply(Xt_model, function(x) apply((x %*% Y_model),1, function(y) sum(y^2)))

    blockrank.i <- lapply(blockrank.i, function(x) x / max(unlist(blockrank.i)))
...
#' }
plot.blockrank.scores <- function(blockrank.i, nscores = 20, model = "", data_source = "") {
    ...
    plot <- ggplot(data=head(plot.data, nscores), aes(x = reorder(feature, blockrank.score), y = blockrank.score, fill=block)) +
        geom_col() +
        theme_minimal() +
        theme(plot.title = element_text(hjust = 0.5) , plot.subtitle = element_text(hjust = 0.5)) +
        scale_fill_brewer(palette = "Set2") +
        labs(x = "Feature", y = "BlockRank Score", title = sprintf("%s model of %s data", model, data_source), subtitle = sprintf("Top %s BlockRank Features", nscores)) +
        coord_flip()
